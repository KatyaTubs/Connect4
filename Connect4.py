        ...
        for _ in range(ROWS*COLS + 1):
            ...
            if board.check_for_win(current_player, actions[current_player]):
                ...
                if game_num % STATUS_SKIP == 0 and game_num != 0:
                    sc1 = self.score[PLAYER1_ID]
                    sc2 = self.score[PLAYER2_ID]
                    per1 = 100.*sc1 / (sc1 + sc2)
                    per2 = 100.*sc2 / (sc1 + sc2)
                    st2print = 'Game %d over:\n' \
                               'Player 1 wins: %d (%.1f)\n' \
                               'Player 2 wins: %d (%.1f)\n' \
                               'Draws: %d' % (game_num, sc1, per1, sc2, per2, self.score[DRAW])
                    # self.logq.put((0, "STATUS", "Game %d " + str(game_num) + " over:\n"
                    #                 "Player 1 wins: " + str(self.score[PLAYER1_ID]) + "\n"
                    #                 "Player 2 wins: " + str(self.score[PLAYER2_ID]) + "\n"
                    #                 "Draws: " + str(self.score[DRAW])))
                    self.logq.put((0, "STATUS", st2print))
